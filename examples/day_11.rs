...
fn expand_space(input: &mut Space, row_mod: Vec<usize>, col_mod: Vec<usize>) {
    let new: Vec<Coordinate> = input.galaxies.iter().map(|(x, y)| {
        (x + col_mod[*x], y + row_mod[*y])
    }).collect();
    input.galaxies = new;
}

fn get_expansion_modifier(bucket: &HashMap<usize, u64>, length: usize, expansion: usize) -> Vec<usize> {
    let mut ret = vec![];
    let mut modifier = 0;
    for i in 0..length {
        match bucket.get(&i) {
        ...
        for (x, c) in line.chars().enumerate() {
            if c == '#' {
                ...
                match row_bucket.get_mut(&y) {
                    Some(v) => *v += 1,
                    None => {row_bucket.insert(y, 1);},
                }
                match col_bucket.get_mut(&x) {
                    Some(v) => *v += 1,
                    None => {col_bucket.insert(x, 1);},
                }
            }
        }
    }
    Space { galaxies, row_bucket, col_bucket , dim: (input.len(), input[0].len())}
}
